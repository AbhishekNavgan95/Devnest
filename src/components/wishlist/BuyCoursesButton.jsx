...
const BuyCoursesButton = ({
    ...
    const buyCourse = async () => {
        try {
            const orderResponse = await api.post('/payment/capturePayment', { courses: courseIds })
            ...
            const options = {
                ...
                handler: (res) => {
                    // verify paymemt
                    const paymentSuccess = varifyPayment(res, courseIds, toast)
                    if (paymentSuccess) {
                        toast({
    ...
    return (
        <Button onClick={buyCourse} className='w-full'>Enroll in all at once</Button>
    )
}

export default BuyCoursesButton